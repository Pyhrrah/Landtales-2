...
char creerSalle(char identifiantSalle, char etage) {
    ...
    if (nbMob > 0) {
        ...
        if ((fichierMob = fopen(nomFichierMob, "w"))==NULL) {
            ...
            fclose(fichierMob);
            fichierMob = NULL;
            //free(mobs);                 //provoque une erreur chez le president lors de l'execution
            mobs = NULL;
            //free(mobsPositions);        //provoque une erreur chez le president lors de l'execution
            mobsPositions = NULL;
        }
    }
...
/*
 quand tu sais pas pourquoi ton code marche sur ton Windows mais pas sur le linux de ton pote
